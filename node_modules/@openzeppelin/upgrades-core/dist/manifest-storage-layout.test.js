...
test.before(async (t) => {
    ...
    assert_1.default(buildInfo !== undefined, 'Build info not found');
    const decodeSrc = src_decoder_1.solcInputOutputDecoder(buildInfo.input, buildInfo.output);
    t.context.validationRun = run_1.validate(buildInfo.output, decodeSrc);
    t.context.validationData = data_1.normalizeValidationData([t.context.validationRun]);
});
test('getStorageLayoutForAddress - update layout', async (t) => {
    const { version } = t.context.validationRun['ManifestMigrateUnique'];
    assert_1.default(version !== undefined);
    const updatedLayout = query_1.getStorageLayout(t.context.validationData, version);
    ...
    const layout = await manifest_storage_layout_1.getStorageLayoutForAddress(manifest, t.context.validationData, address);
...
test('getUpdatedLayout - unique layout match', async (t) => {
    const { version } = t.context.validationRun['ManifestMigrateUnique'];
    assert_1.default(version !== undefined);
    const targetLayout = query_1.getStorageLayout(t.context.validationData, version);
    const outdatedLayout = removeStorageLayoutMembers(targetLayout);
    const updatedLayout = manifest_storage_layout_1.getUpdatedStorageLayout(t.context.validationData, version.withoutMetadata, outdatedLayout);
...
test('getUpdatedLayout - multiple unambiguous layout matches', async (t) => {
    ...
    assert_1.default(version1 !== undefined && version2 !== undefined);
    ...
    const targetLayout = query_1.getStorageLayout(t.context.validationData, version1);
    const outdatedLayout = removeStorageLayoutMembers(targetLayout);
    const updatedLayout = manifest_storage_layout_1.getUpdatedStorageLayout(t.context.validationData, version1.withoutMetadata, outdatedLayout);
    t.deepEqual(updatedLayout, targetLayout);
});
