    ...
    for (const varDecl of contractDef.nodes) {
        if (utils_1.isNodeType('VariableDeclaration', varDecl)) {
            if (!varDecl.constant && varDecl.mutability !== 'immutable') {
                const type = type_id_1.normalizeTypeIdentifier(typeDescriptions(varDecl).typeIdentifier);
                layout.storage.push({
                ...
                assert_1.default(varDecl.typeName != null);
                ...
                for (const typeName of typeNames.values()) {
                    ...
                    const type = type_id_1.normalizeTypeIdentifier(typeIdentifier);
...
const findTypeNames = utils_1.findAll([
    'ArrayTypeName',
    'ElementaryTypeName',
    'FunctionTypeName',
    'Mapping',
...
function typeDescriptions(x) {
    assert_1.default(typeof x.typeDescriptions.typeIdentifier === 'string');
    assert_1.default(typeof x.typeDescriptions.typeString === 'string');
    return x.typeDescriptions;
}
function getTypeMembers(typeDef) {
    if (typeDef.nodeType === 'StructDefinition') {
        return typeDef.members.map(m => {
            assert_1.default(typeof m.typeDescriptions.typeIdentifier === 'string');
            return {
                label: m.name,
                type: type_id_1.normalizeTypeIdentifier(m.typeDescriptions.typeIdentifier),
            };
        });
    }
    else {
