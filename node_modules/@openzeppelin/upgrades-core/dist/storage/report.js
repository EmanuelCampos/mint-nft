...
class LayoutCompatibilityReport {
    ...
    explain() {
        ...
        for (const op of this.ops) {
            ...
            res.push(chalk_1.default.bold(src) + ':' + indent_1.indent(explainStorageOperation(op, { kind: 'layout', allowAppend: true }), 2, 1));
        }
        return res.join('\n\n');
    }
}
...
function explainTypeChange(ch) {
    switch (ch.kind) {
        ...
        case 'mapping key':
            return `In key of ${ch.updated.item.label}\n` + itemize_1.itemize(explainTypeChange(ch.inner));
        case 'mapping value':
        case 'array value':
            return `In ${ch.updated.item.label}\n` + itemize_1.itemize(explainTypeChange(ch.inner));
        case 'array shrink':
        case 'array grow': {
            assert_1.assert(ch.original.tail && ch.updated.tail);
...
function explainTypeChangeDetails(ch) {
    switch (ch.kind) {
        case 'struct members': {
            ...
            return (`In ${ch.updated.item.label}\n` +
                itemize_1.itemize(...ch.ops.map(op => explainStorageOperation(op, { kind: 'struct', allowAppend }))));
        }
        case 'enum members':
            return `In ${ch.updated.item.label}\n` + itemize_1.itemize(...ch.ops.map(explainEnumOperation));
    }
}
