...
function normalizeManifestData(input) {
    return {
        ...pick_1.pick(input, ['manifestVersion', 'admin']),
        proxies: input.proxies.map(p => normalizeDeployment(p, ['kind'])),
        impls: map_values_1.mapValues(input.impls, i => i && normalizeDeployment(i, ['layout'])),
    };
}
function normalizeDeployment(input, include = []) {
    return pick_1.pick(input, ['address', 'txHash', ...include]);
}
