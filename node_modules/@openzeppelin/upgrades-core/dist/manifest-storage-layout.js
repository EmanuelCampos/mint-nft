...
async function getStorageLayoutForAddress(manifest, validations, implAddress) {
    ...
    if (extract_1.isCurrentLayoutVersion(layout)) {
        return layout;
    }
    else {
...
function getUpdatedStorageLayout(data, versionWithoutMetadata, layout) {
    ...
    outer: for (const [contractName, validationRun] of query_1.findVersionWithoutMetadataMatches(data, versionWithoutMetadata)) {
        const updatedLayout = query_1.unfoldStorageLayout(validationRun, contractName);
        ...
        for (const [i, item] of layout.storage.entries()) {
            ...
            if (type_id_1.stabilizeTypeIdentifier(item.type) !== type_id_1.stabilizeTypeIdentifier(updatedItem.type)) {
                continue outer;
            }
        }
    ...
    if (sameLayout.size > 1) {
        ...
        for (const { types } of sameLayout.values()) {
            for (const [typeId, item] of Object.entries(types)) {
                ...
                const members = layout_1.isStructMembers(item.members)
                    ? item.members.map(m => [type_id_1.stabilizeTypeIdentifier(m.type), m.label])
                    : item.members;
                const stableId = type_id_1.stabilizeTypeIdentifier(typeId);
                if (!typeMembers.has(stableId)) {
                    typeMembers.set(stableId, members);
                }
                else if (!deep_array_1.deepEqual(members, typeMembers.get(stableId))) {
                    return undefined;
                }
            }
        }
